...
object SwaggerOperation {
  ...
  def simple(method: HttpMethod, nickname: String, parameters: Seq[SwaggerParameter]) =
    new SwaggerOperation(method, nickname, nickname, parameters)
}
...
class SwaggerRestDocumentation(
    val apiPrefix: String,
    val restApi: RestRouter,
    val globalParameters: Seq[SwaggerParameter] = Seq(),
    val apiVersion: String = "1.2"
)(actionBuilder: ActionBuilder[Request, AnyContent], assets: controllers.Assets)
    extends RouterWithPrefix {
  ...
  def operationList(
      path: String,
      routeInfo: RestRouteInfo,
      parameters: Seq[SwaggerParameter] = Seq()
  ): List[SwaggerApi] = {
    ...
    var ops = routeInfo.caps.flatMap { caps =>
      caps match {
        ...
        case ResourceCaps.Create =>
          Some(SwaggerOperation.simple(POST, s"Create ${routeInfo.name}", parameters :+ bodyParam))
        case ResourceCaps.Action =>
          routeInfo.asInstanceOf[ActionRestRouteInfo].methods.map { method =>
            SwaggerOperation.simple(
              method,
              routeInfo.name,
              if (Seq(POST, PUT, PATCH).contains(method)) parameters :+ bodyParam
              else parameters
            )
          }
        case _ => None
    ...
    ops = routeInfo.caps.flatMap { caps =>
      caps match {
        ...
        case ResourceCaps.Write =>
          Some(SwaggerOperation.simple(PUT, s"Write ${routeInfo.name}", subParams :+ bodyParam))
        case ResourceCaps.Update =>
          Some(SwaggerOperation.simple(PATCH, s"Update ${routeInfo.name}", subParams :+ bodyParam))
    ...
    res ++ routeInfo.subResources.flatMap(info =>
      operationList(s"$path/{${routeInfo.name}_id}/${info.name}", info, subParams)
    )
  }

  def resourceListing = actionBuilder {
    val res = Json.obj(
  ...
  def renderSwaggerUi(prefix: String) = actionBuilder {
    Results.Ok(views.html.swagger(prefix + ".json", prefix + "/ui"))
  }

  def resourceDesc(path: String, routeInfo: RestRouteInfo) = actionBuilder {
    val res = Json.obj(
      "apiVersion" -> apiVersion,
      "swaggerVersion" -> "1.2",
      "basePath" -> apiPrefix,
  ...
  override def routesWithPrefix(prefix: String) = scala.Function.unlift { requestHeader =>
    ...
    requestHeader.path match {
      ...
      case UiAsset(asset) =>
        Some(assets.at(path = s"/META-INF/resources/webjars/swagger-ui/${swaggerUiVersion}", file = asset))
      case _ => None
    }
  }
